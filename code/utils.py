...
def plot_losses(losses, exDir, factor, title='loss'):
	...
	for key in losses:
		...
		plt.plot(np.arange(len(losses[key]))*factor[key],losses[key], label=key)
...
def plot_log_losses(losses, exDir, epochs=1, title='loss'):
	#losses should be a dictionary of losses 
	# e.g. losses = {'loss1':[], 'loss2:'[], 'loss3':[], ... etc.}
	fig1 = plt.figure()
	assert epochs > 0
	...
	plt.legend()
	plt.title(title)
	fig1.savefig(join(exDir, 'log_' +title+'_plt.png'))
	plt.close(fig1)
